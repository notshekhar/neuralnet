...
class NeuralNetwork {
    constructor(arr, lr) {
        ...
        for (let i = 0; i < this.nodes.length - 1; i++) {
            this.weights.push(
                new Matrix(this.nodes[i + 1], this.nodes[i]).randomize()
            )
        }
        ...
        for (let i = 0; i < this.weights.length; i++) {
            for (let j = 0; j < this.weights[i].rows; j++) {
                for (let k = 0; k < this.weights[i].cols; k++) {
                    this.weights[i].data[j][k] =
                        this.weights[i].data[j][k] * r1 +
                        net.weights[i].data[j][k] * r2
                }
            }
        }
        for (let i = 0; i < this.biases.length; i++) {
            for (let j = 0; j < this.biases[i].rows; j++) {
                for (let k = 0; k < this.biases[i].cols; k++) {
                    this.biases[i].data[j][k] =
                        this.biases[i].data[j][k] * r1 +
                        net.biases[i].data[j][k] * r2
                }
            }
        }
        return this
    ...
    crossover(net) {
        ...
        this.lr = Math.random() > 1 / 2 ? this.lr : net.lr
